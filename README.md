...
<br>只需要配置相应的视频服务器地址，应用服务器地址，流媒体服务器地址，娃娃机就可以正确联网并推流视频。
<br>通过我们提供的简单客户端（SimpleClient）和服务器(SimpleServer)，您可以快速的了解如何操作雪暴网络娃娃机以及如何开发您的应用服务器。定制您自己的网络娃娃机项目。
<br>我们有提供公网，局域网，本机，升级，安装，管理的一系列工具软件，该软件将随着版本升级越来越好用，关于本配套软件，雪暴公司欢迎用户提出反馈以便使其更加易用。
<br>如果您选择android-push.apk，而决定采用自己的管理软件，我们会在稍后放出管理协议。目前由于部分功能仍在开发，因此接口尚不完备.
<br>我们还在加紧整理文档和开发新功能和用户使用指引。敬请关注本github的说明及wiki。

2.如果您不选用公有的RTMP视频推流格式，或您自己选择了其他第三方更加快速的私有视频协议，您也可以购买我们的产品，然后选择不安装android-push.apk。
<br>此时您必须:开发自己的推流apk。 在您的apk中，您将需要
<br>)根据网络娃娃机主板发来的命令处理数据并转发给应用服务器。该文档对应document.docx.后续我们会将其更新到本github的wiki页面
<br>)自己采集摄像头的数据，并区分绑定前后摄像头，然后将数据编码并发送给您的(流媒体，或其他协议服务器)。

您还将需要
<br>自己开发配套的协议来标识并管理您的娃娃机。
