...
使用雪暴娃娃机的安卓版套装，您可以
<br>
1.通过我们提供的android-push.apk快速拥有一个完整的支持双路视频RTMP推流并使娃娃机联网的解决方案。配合** <a href="https://github.com/daniulive/SmarterStreaming" target="_blank">大牛直播SDK播放端</a>**，将可以使视频延迟保证在一秒左右。
...
* 我们有提供公网，局域网，本机，升级，安装，管理的一系列工具软件，该软件将随着版本升级越来越好用，关于本配套软件，雪暴公司欢迎用户提出反馈以便使其更加易用。
* 如果您选择android-push.apk，而决定采用自己的管理软件，我们会在稍后放出管理协议。目前由于部分功能仍在开发，因此接口尚不完备.
* 我们还在加紧整理文档和开发新功能和用户使用指引。敬请关注本github的说明及wiki。

2.**本安卓版也支持各种互动直播技术，如腾讯互动直播，阿里连麦，即构，anyRCT的私有视频传输协议。**。如果选择私有视频协议，您也可以购买我们的产品，然后选择不安装android-push.apk。
<br>此时您必须:开发自己的推流apk。 在您的apk中，您将需要
* 根据网络娃娃机主板发来的命令处理数据并转发给应用服务器。该文档对应[**网络娃娃机主板对接协议.docx**].后续我们会将其更新到本github的wiki页面
* 自己采集摄像头的数据，并区分绑定前后摄像头，然后将数据编码并发送给您的(流媒体，或其他协议)服务器。

您还将需要
* 自己开发配套的协议来标识并管理您的娃娃机。

**您也可以选择采用即构，anyRTC等已经实现完整的私有协议，他们已完整的对接好我们的整套硬件，提供直接可用的整套产品。包含推流端、服务器端、播放端、操作客户端**


**安卓板与娃娃机指令交互说明**文件夹里面包含了第二种方案时，如何跟娃娃机对接的简单封装类。具体的协议实现，您必须参照document.docx去自行实现。


本项目是雪暴网络娃娃机安卓版使用说明及例子源码。包含安卓推流端安装程序及配套的配置工具软件，简单应用服务器示例源码，简单操作客户端示例源码，娃娃机协议文档。
...
2.使用prebuild中的android-push文件夹中的安装脚本将android-push.apk安装到安卓板上，这时，如果网线已连接到路由器，则该娃娃机已正常工作。
...
* [**SimpleServer**]简单应用服务器java源码。
	...
	* 应用服务器 （默认端口  对于安卓板android-push.apk是7770，对于玩家手机客户端simpleClient.apk是7771）
	* 配置服务器 （默认端口 对于安卓板android-push.apk是7776，对于公网配置工具是7778）
	* android-push.apk启动后，会自动注册到配置服务器，外网配置工具可通过配置服务器配置安卓板参数
	* 目前没做用户认证，因此多个用户同时修改一个娃娃机的配置时，只有最后一个配置者的命令会生效。

* [**安卓板与娃娃机指令交互说明**] 当您决定不使用android-push推流标准RTMP或者使用第三方私有协议实现推流，文件夹包含了对接娃娃机主板的核心代码。方便您自己实现推流程序
